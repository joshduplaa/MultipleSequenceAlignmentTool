...
def Task_5(alignedSequences, gapPenalty, score_matrix):
    columnScoreList = []
        ...
        for j in range(len(alignedSequences)): #loops through the ith charcter of every sequeence or "row"
            ...
            if current != "-":
                for k in range(j+1, len(alignedSequences)):
                    ...
                    if current == compareTo:        #match
                        columnScore += matchScore
                    elif compareTo == "-":          #gap     
                        columnScore += gapPenalty
                    else:                           #mismatch
                        columnScore += mismatchScore
        
                    print(compareTo)
        columnScoreList.append(int(columnScore))
                    
    msaScore = sum(columnScoreList)
    # discuss with Rees, alternative strategy: Transpose the aligned Sequences (Columns become rows), operate on each row. I think that may be slower though
